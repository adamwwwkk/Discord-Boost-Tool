...
capmonster_key = "d1c66b006fce96d4243ae7bab3fdb7c3"
activity = ".gg/generator"
server_nick = ".gg/generator"
change_server_nick = False
proxyless = True
...
os.system(f"title boost tool by discord.gg/firegen, author (peak.z)")    
...
def get_fingerprint(thread):
    try:
        fingerprint = httpx.get(f"https://discord.com/api/v10/experiments", proxies =  {'http://': f'http://{random.choice(open("input/proxies.txt", "r").read().splitlines())}', 'https://': f'http://{random.choice(open("input/proxies.txt", "r").read().splitlines())}'} if proxyless != True else None)
...
def boost_server(invite:str , months:int, token:str, thread:int, nick: str):
    ...
    try:
        ...
        if proxyless == False and len(open("input/proxies.txt", "r").readlines()) != 0:
            proxy = getproxy()
...
▄████████  ▀█████▀  ▀█████▀ ██████▀ ▀████      ▄████▄    ▀█████▀  ▀█████▀ ▄████▄
                  ...
                  @peak.z   .gg/firegen
''')
print(f'''{Style.BRIGHT}{Fore.MAGENTA}                  1m Boost Stock: {len(open('input/1m_tokens.txt', 'r').read().splitlines()) * 2}''' + Fore.RESET)
print(f'''{Style.BRIGHT}{Fore.MAGENTA}                  3m Boost Stock: {len(open('input/3m_tokens.txt', 'r').read().splitlines()) * 2}\n''' + Fore.RESET)	
